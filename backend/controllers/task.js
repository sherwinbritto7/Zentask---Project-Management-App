...
const createTask = async (req, res) => {
  try {
    ...
    // Sync project members with task assignees
    if (assignees && assignees.length > 0) {
      assignees.forEach((assigneeId) => {
        const isAlreadyProjectMember = project.members.some(
          (m) => m.user.toString() === assigneeId.toString()
        );
        if (!isAlreadyProjectMember) {
          project.members.push({ user: assigneeId, role: "contributor" });
        }
      });
    }
  ...
  try {
    ...
    const isPrivileged =
      ...
      workspace.members?.some((m) => {
        const memberIdStr = m.user?.toString();
        return (
          memberIdStr === userIdStr &&
          ["admin", "owner"].includes(m.role)
        );
      });
    ...
    project.tasks = project.tasks.filter((t) => t && t.toString() !== taskId);
    await project.save();
